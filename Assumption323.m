function y=Assumption323(param,temps,fixedParam)
...
function dy=G(t,X)
    ...
    dy(2)=(HOC*X(1)*(X(5)+X(7)+X(8))/OBini/(1+M2Inh*X(8)))-deltaOCFixed*(1+deltaOC*X(8))*X(2);
    dy(3)=-deltaBFixed*(1+TNF*(X(6)+X(7)))*X(2)*X(3)/BoneIni+piBFixed*(1+OSM*(X(8)))*(X(1));
    dy(4)=monoInflux2*X(9)/WoundVol+monoInflux1*(X(7)+X(6))/(MacIni+MonoIni)+H-deltaMo*X(4)-Factor1*X(4)*X(7)/((MacIni+MonoIni))+depol1*X(6);
    dy(5)=(d)*MonoIni-(HOC*X(1)*X(5))/OBini/(1+M2Inh*X(8))-Factor2*X(5)*X(9)/(WoundVol*(MacIni+MonoIni))-(deltaM*X(5));
    dy(6)=Factor1*X(4)*X(7)/((MacIni+MonoIni))-depol1*X(6)-deltaMo*X(6);
    dy(7)=Factor2*X(5)*X(9)/(WoundVol*(MacIni+MonoIni))-deltaM*X(7)-AntiInfl*X(7)*X(9)+depol3*X(8)-(HOC*X(1)*X(7))/OBini/(1+M2Inh*X(8));
    dy(8)=AntiInfl*X(7)*X(9)-deltaM*X(8)-depol3*X(8)-(HOC*X(1)*X(8))/OBini/(1+M2Inh*X(8));
